...
getElement('#settings').onclick = function() {
	...
	HUD.MENU.KEYBOARD.preventBackSpace = false;
	HUD.MENU.RAY.DESTROY()
	SYS.DOM.E('RTCNODEJS').style.height = window.innerHeight + 'px';
};

getElement('#save-settings').onclick = function() {

   SYS.DOM.E('RTCNODEJS').style.height =  '150px';
   HUD.MENU.KEYBOARD.preventBackSpace = true;
   HUD.MENU.CREATE_RAY()
