...
function addNewMessage(args) {
	...
	//var newMessageDIV = document.getElementsByClassName('new-message')[0];

     newMessageDIV.className = 'new-message';

    //var userinfoDIV = document.createElement('div');
	 var userinfoDIV = document.getElementsByClassName('user-info')[0];
    userinfoDIV.className = 'user-info';
	
    userinfoDIV.innerHTML = args.userinfo || '<p>MSG:</p>';
       
    userinfoDIV.style.background = args.color || rtcMultiConnection.extra.color || getRandomColor();
    
	   if (typeof rtcMultiConnection.extra.color != 'undefined') {
	   if (typeof rtcMultiConnection.extra.color == "#61463A" ) {
   ...
   //var userActivityDIV = document.getElementsByClassName('user-activity')[0];

   //alert(userActivityDIV)
...
getElement('.main-input-box textarea').onkeyup = function(e) {
    ...
    addNewMessage({
        ...
        userinfo: getUserinfo(rtcMultiConnection.blobURLs[rtcMultiConnection.userid], 'lib/rtc/html/images/chat-message.png'),
