...
// Setup our world
export function initCannon(scene, RM) {
  ...
  ROOT.AUTO_UPDATE = function () {
      ...
      for (var i = 0; i < CANNON_OBJECTS.boxes.length; i++) {
          // console.log('TEST', CANNON_OBJECTS.boxMeshes[i].position)
  ...
  ROOT.CREATE_BOX_CANNON = function (position, dim) {
      ...
      var halfExtents = new CANNON.Vec3(110, 10, 10);
      var boxShape = new CANNON.Box(halfExtents);
      var boxGeometry = new THREE.BoxGeometry(halfExtents.x * 2, halfExtents.y * 2, halfExtents.z * 2);
      var boxBody = new CANNON.Body({
              mass: 50
      ...
      boxBody.addShape(boxShape);
      console.log("TEST RM ", RM)
      let boxMesh = new THREE.Mesh(boxGeometry, RM.PhongMaterial);
      ROOT.world.addBody(boxBody);
      scene.add(boxMesh);
      CANNON_OBJECTS.boxes.push(boxBody);
      CANNON_OBJECTS.boxMeshes.push(boxMesh);
